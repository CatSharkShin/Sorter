...
if len(sys.argv) == 1:
    print("\nDescription: This script sorts files in the directory it is in by file extension. It also creates directories needed for the sort"
          ...
          "\n-c : Sorts code files"
          "\n-r : Resets everything"
        "\n\nLeírás: Ez a program fájlok rendezésére alkalmas, fájlkiterjesztés alapján csoportosítja �ket mappákba. Ezeket a mappákat szükségszerűen létre is hozza"
          "\n\n�sszegzés: sorter.py [OPCI�]..."
          "\n\nOpciók:"
    ...
    if len(sys.argv) == 2 and '-r' in args:
        ...
        for file in [file for file in files if not os.path.isdir(file) and not file == __file__]:
            dest = ""
